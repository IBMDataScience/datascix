...
const ERROR_INVALID_AUDIENCE = "Audience provided is not a valid array";

/* eslint-disable max-len */

const urlRegex = /^http[s]?:\/\/(?:www\.)?[-a-zA-Z\d\.]+?(?::[\d]+?)?(?:\/[-A-Za-z\d\/$_\.\+!\*'\(\),%]+?(?:\?[-A-Za-z\d$_\.\+!\*'\(\),%;:@&=]+)?)?$/;
...
function _validateEntry(entry, index) {
	...
	if (!_isValidAudienceList(audience)) {
		return prefix + ERROR_INVALID_AUDIENCE;
	}
...
function _isValidAudienceList(object) {
	if (!Array.isArray(object)) {
		return false;
	}

	for (let idx = 0; idx < object.length; idx++) {
		if (VALID_AUDIENCES.indexOf(object[idx]) === -1) {
			return false;
		}
	}
	return true;
