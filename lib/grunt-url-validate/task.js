...
const REQUIRED_ENTRY_PROPERTIES = [ID, AUTHOR, TITLE, DATE, MESSAGE, IMAGE_URL, BLOG_URL];

const ERROR_BODY_NOT_ARRAY = "JSON body must be a defined Array.";
const ERROR_ENTRY_NOT_OBJECT = "Each entry must be a defined Object.";
const ERROR_INVALID_ID = "The id property is undefined or invalid.";
const ERROR_INVALID_GUID = "The id property is not a valid GUID.";
const ERROR_INVALID_TITLE = "The title property is undefined or invalid.";
const ERROR_INVALID_AUTHOR = "The author property is undefined or invalid.";
const ERROR_INVALID_DATE = "The date property is undefined or invalid";
const ERROR_INVALID_MESSAGE = "`Message` must be a defined String.";
const ERROR_INVALID_URL = "URL provided is not a valid URL syntactically";
const ERROR_INSECURE_URL = "URL is not secure (i.e. Must be HTTPS)";
...
function _validateEntry(entry, index) {
	...
	if (!_isURL(imageUrl)) {
		return prefix + IMAGE_URL + ERROR_INVALID_URL;
	}

	if (!_isSecureURL(imageUrl)) {
		return prefix + IMAGE_URL + ERROR_INSECURE_URL;
	}

	if (!_isURL(blogUrl)) {
		return prefix + BLOG_URL + ERROR_INVALID_URL;
	}
...
function _doesUrlExist(index, title, url, callback, grunt) {
	...
	request(url, function(err, res, body) {
		...
		if (err) {
			...
			const message = prefix + `Failed to retrieve URL: ${url}`;
...
function _validateURLs(entries, done, grunt) {
	...
	for (const entry of entries) {
		...
		asyncTasks.push(function(callback) {
			_doesUrlExist(index, title, imageUrl, callback, grunt);
		});

		asyncTasks.push(function(callback) {
			_doesUrlExist(index, title, blogUrl, callback, grunt);
		});
